...
\begin{align}
  ...
  \begin{array}{c}
      ...
      0 \\
      v_0
  \end{array}\right) =\left(\bm{3}, 1,-\frac{1}{3}\right)\oplus\left(1,\bm{2},\frac{1}{2}\right),\quad(v_0\sim 246\,[\mathrm{GeV}])
\end{align}
のように標準模型と同じ値を取る.
もし24表現ヒッグスと5表現ヒッグスに何らかの交わりがない場合, ポテンシャルは
\begin{align}
  V(\phi_{\bm{5}}) = -{\mu_5}^2\phi_{\bm{5}}^\dagger \phi_{\bm{5}} + \frac{\lambda}{4}(\phi_{\bm{5}}^\dagger \phi_{\bm{5}})^2
\end{align}
である.
%NOTE: Notationが微妙にちがう. BEGNに合わせると \frac{\mu^2}{2}になる.
\subsection{予言される現象}
\subsection{破綻}
